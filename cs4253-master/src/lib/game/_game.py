...
class Game:
	...
	def run(self, play_again='query', speed=2):
		e = 0
		while True and e < EPISODES:
			self._run_round(speed, e)
			e += 1
			if not play_again or play_again == 'query' and not self._play_again():
				break

	def _run_round(self, speed, e = 0):
		...
		while not state.is_terminal:
			...
			while new_state == None:
				...
				agent = self.agents[i]
				#agent = Agent(self.agents[i].evaluate,self.agents[i].alpha_beta_pruning, self.agents[i].max_depth, self.agents[i].dqnAgent)
				...
				if dqn:
					reward = 10 if new_state.is_terminal and state.winner == state.current_player else -10
					Sstate = agent.RnpState(state)
					Snew_state = agent.RnpState(new_state)
					agent.dqnAgent.remember(Sstate, agent.predictMinmax(state, i), reward, Snew_state, new_state.is_terminal)
					batch_size = 32
		...
		for player_id, agent in enumerate(self.agents):
			...
			if dqn:
				print("episode: {}/{}, score: {}, e: {:.2}".format(e, EPISODES, reward, agent.dqnAgent.epsilon))
				if len(agent.dqnAgent.memory) > batch_size:
					agent.dqnAgent.replay(batch_size)
				if e % 10 == 0:
					print("Saving...")
					agent.dqnAgent.save("./save/discrete_soccer"+ str(player_id)+".h5")
...
class GameState(PRecord):
	...
	def _action_is_valid(self, action):
		if self.is_terminal:
			print(
				"""ERROR: Player tried to perform an action in a terminal state. Make sure that you are performing actions solely in non-terminal states.""")
			return None
